document.addEventListener('DOMContentLoaded', function() {
  loadFileContents('lib/json/game-creation-cookie-template.json').then(function(res) {
    ...
    setCookie("GameInitCookie",json,0.0069);
...
function openNumPlayerSelect(data) {
  loadFile('templates', 'template/num-player-select.html').then(function(res) {
    ...
    if (data !== null) {
      ...
      for (var i in inputs) {
        if ((iLabel + data.num) === inputs[i].id) {
...
function readPlayerNumbers() {
  ...
  addHistory('numPlayers', {"num": numPlayers});
  return numPlayers;
}

// Launch entering player names interface
function enterPlayerNames(numPlayers, data) {
  var content = document.getElementById('content');
  content.innerHTML = '';
...
function readPlayerNames() {
  ...
  addHistory('playerNames', {"players": names});
}
