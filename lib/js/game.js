    ...
    loadFileContents('lib/json/game-list.json').then(function(gameRes) {
      ...
      loadFileContents('lib/json/' + gameTemp).then(function(res) {
        ...
        for (var index = 0; index < numPlayers; index++) {
          ...
          appendChildNodes(playerTmp, playerTabs);
        }
        playerTabs.childNodes[0].getElementsByClassName('player')[0].classList.add('active');
...
function increment(elem){
  ...
  // var value = elem.parentNode.getElementsByClassName('counter-value')[0];
...
function activatePlayer() {
  ...
  for (var i = 0; i < tabs.childElementCount; i++) {
    var elem = tabs.childNodes[i].getElementsByTagName('input')[0];
    var player = elem.parentNode.getElementsByClassName('player')[0];
