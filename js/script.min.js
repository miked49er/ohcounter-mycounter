...
function checkCookie() {
    ...
    "" != a ? alert("Welcome again " + a) : "" != (a = prompt("Please enter your name:", "")) && null != a && setCookie("username", a, 365);
}
...
function openNumPlayerSelect(a) {
    loadFile("templates", "template/num-player-select.html").then(function(b) {
        ...
        if (c.innerHTML = "", newTemplate("num-players-template", [], c), null !== a) {
            var d = document.getElementsByName("player-selection");
            for (var e in d) "player-option-" + a.num === d[e].id && (d[e].checked = !0);
        } else newState("numPlayers");
    }, function(a) {
...
function readPlayerNumbers() {
    var a = document.getElementsByName("player-selection"), b = whichIsChecked(a).value;
    return addHistory("numPlayers", {
        num: b
    }), b;
}

function enterPlayerNames(a, b) {
    var c = document.getElementById("content");
    c.innerHTML = "", newTemplate("player-names-template", [], c);
    for (var d = document.getElementsByClassName("player-names-input")[0], e = 0; e < a; e++) {
        ...
        if (g.setAttribute("for", "player" + e), g.setAttribute("id", "playerLabel" + e), 
        null !== b) {
            var h = b.players[e];
            f.value = void 0 != h ? h : "";
        }
    }
    null === b && newState("playerNames");
}
...
function domLoad(a) {
    ...
    switch (console.log("pop: " + b), b) {
      ...
      case "playerNames":
        enterPlayerNames(c.players.length, c);
        break;
...
function addHistory(a, b) {
    ...
    console.log("replace: " + a), history.replaceState(c, null, null);
}
...
function newState(a) {
    ...
    console.log("push: " + a), history.pushState(b, null, null);
}
...
document.addEventListener("DOMContentLoaded", function() {
    openNumPlayerSelect(null), window.addEventListener("popstate", function(a) {
        domLoad(a.state);
    });
});