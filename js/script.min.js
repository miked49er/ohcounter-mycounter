...
function checkCookieContents(a, b) {
    ...
    return JSON.stringify(b).valueOf() !== c.valueOf();
}
...
function setUpGame(a) {
    loadFile("templates", "template/game.html").then(function(b) {
        ...
        for (var h = 0; h < c; h++) {
            ...
            for (var w = i.secondary, x = 0; x < w.length; x++) {
                var y = w[x];
            ...
            for (var B = i.tertiary, C = 0; C < B.length; C++) {
                var D = B[C];
...
function createCounterGroup(a, b, c) {
    var d = convertToElement(document.getElementById("increment-group-template").innerHTML);
    loss = d.getElementsByClassName("loss")[0], loss.innerHTML = a[p], loss.dataset.counter = b, 
    gain = d.getElementsByClassName("gain")[0], gain.innerHTML = a[p], gain.dataset.counter = b, 
