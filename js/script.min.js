    ...
    loadFile("templates", "template/game.html").then(function(b) {
        ...
        for (var h = 0; h < c; h++) {
            ...
            if (p.increments.length > 1) {
                ...
                for (var q = n.getElementsByClassName("multi-counter")[0], r = 0; r < p.increments.length; r++) createCounterGroup(p.increments[r], p.id, q);
                appendChildNodes(n, m);
            } else createCounter("p-single-template", p, m);
            for (var s = i.secondary, t = 0; t < s.length; t++) {
                var u = s[t];
                if (u.increments.length > 1) {
                    n = convertToElement(document.getElementById("s-multi-template").innerHTML), n.getElementsByClassName("counter-name")[0].innerHTML = u.name, 
                    o = n.getElementsByClassName("counter-value")[0], o.innerHTML = u.start, addDataset(o, u);
                    for (var v = n.getElementsByClassName("multi-counter")[0], w = 0; w < u.increments.length; w++) createCounterGroup(u.increments[w], u.id, v);
                    appendChildNodes(n, m);
                } else createCounter("s-single-template", u, m);
            }
            for (var x = i.tertiary, y = 0; y < x.length; y++) {
                createCounter("t-single-template", x[y], m);
            }
...
function createCounterGroup(a, b, c) {
    ...
    loss = d.getElementsByClassName("loss")[0], loss.innerHTML = a, loss.dataset.counter = b, 
    gain = d.getElementsByClassName("gain")[0], gain.innerHTML = a, gain.dataset.counter = b, 
